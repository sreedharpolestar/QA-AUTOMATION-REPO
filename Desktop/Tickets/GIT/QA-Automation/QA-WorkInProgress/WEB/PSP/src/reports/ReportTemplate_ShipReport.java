...
public class ReportTemplate_ShipReport extends BasePage {
	...
	public ReportTemplate_ShipReport(WebDriver driver) {
		super(driver);
	}
...
// Report template module navigation and verifying the header
	...
	@Test
	public static void reportTemplate(WebDriver driver) throws Throwable{
...
//Verifying the Report templates header text
			
			String Reports_header_exp = "Report templates";
		
			String Reports_header_act = Reports.reportTemplates_homepage_header(driver).getText();
			Assert.assertEquals(Reports_header_exp, Reports_header_act);
			log.info("Report templates  header text is getting deisplayed as :  " + Reports_header_act);
			 
//Verifying Create a new report template button
	
	String Createanewreport = "Create a new report template";
	
	String create_report= Reports.createNewReportTemplate_button(driver).getText();
...
//Verifying Create a new report template page after clicking on "Create a new report template" button

	//header text validation
	String Create_new_report_exp = "Create a new report template";
	String Create_new_report_act= Reports.createNewReportTemplate_header(driver).getText();
	Assert.assertEquals(Create_new_report_exp, Create_new_report_act);
	log.info("Create new report template page header is getting displayed as : " + Create_new_report_act);
	
	//Sub header text validation
		String Create_new_report_subtitle_exp = "Choose a template based on which you want to create a new template";
		String Create_new_report_subtitle_act= Reports.createNewReportTemplate_subheader(driver).getText();
		Assert.assertEquals(Create_new_report_subtitle_exp, Create_new_report_subtitle_act);
		log.info("Create new report template page sub title is getting displayed as : " + Create_new_report_subtitle_exp);
		
		
	//Clicking on New ship report link
		
		Reports.createNewReportTemplate_ShipReport_link(driver).click();
		
		//New ship report template page validation
		...
		log.info("Create new ship report template page header is getting displayed as : " + Create_new_report_act1);
		
		//Sub header text validation
			String Create_new_report_subtitle_exp1 = "Create a report template based on New ship report report model";
		...
		//Template name validation
		    ...
		    driver.findElement(By.cssSelector("#form__item__name")).sendKeys("Ship Template");
		    
		    String shipTemplateName = driver.findElement(By.cssSelector("#form__item__name")).getText();
...
//Creating the new Ship report by selecting/filling all the required details
	...
	//Randomly selecting the options from the drop down		
			...
			if(random == 1){
				select.selectByVisibleText("Ship Status");
			}else if(random == 2){
				select.selectByVisibleText("Ship Name");
			}else{
				select.selectByVisibleText("Fleet Name");
			}
	...
	if(selected_Filter_option == "Ship Status"){
		...
		//*[@id="application"]/div[2]/div[2]
		 ...
		 driver.findElement(By.xpath("//div[2]/div[2]/div[2]/div[2]/div/div[2]/div/form/div/div[3]/div[2]/div[3]/input")).sendKeys("AAA");
		...
		WebElement dropdown3 = Reports.create_new_report_filter_ShipReports_NewShipReport_Filters_exactly(driver);
		 ...
		 Thread.sleep(3000);
		     ...
		     driver.findElement(By.xpath("//div/form/div/div[4]/div[1]/div[3]/input")).sendKeys("9999");
...
//Verifying whether Template is created or not
	...
	Reports.createNewReportTemplate_Reporttemplate_search(driver).sendKeys(shipTemplateName);
	
	String shipreportName_found = Reports.createNewReportTemplate_Reporttemplate_createdTemplate_namelink(driver).getText();
	
	Assert.assertEquals(shipreportName_found, shipTemplateName);
		
	Log.info("Ship report template is generated successfully");	

	
//Validating the created ship report template details
	...
	String shipreport_Header_act = driver.findElement(By.xpath("//div[2]/div[2]/div/div[1]/div/div/h3")).getText();
	
	
	Assert.assertEquals(shipTemplateName, shipreport_Header_act);
	...
	Assert.assertEquals(Template_name, shipTemplateName);
	...
	if(dropdown_filters_1row_1column_act == "Ship Status"){
	...
	}else if(dropdown_filters_1row_1column_act == "Ship Name"){
		...
		WebElement dropdown_filters_1row_2column1 = driver.findElement(By.xpath("//div[2]/div[2]/select"));
		 ...
		 Assert.assertEquals(Filters_1row_3column_text, "AAA");
		 
		Log.info("Filters section first row details are correct when Ship name is selected");
		
		 Thread.sleep(3000);
		
	}else if(dropdown_filters_1row_1column_act == "Fleet Name"){
...
//Updating the Template 
	...
	driver.findElement(By.cssSelector("#form__item__name")).sendKeys("edit_Ship Template");
	
	String Edit_ShipTemplate_name = driver.findElement(By.cssSelector("#form__item__name")).getText();
	
	//editing Template group 
	
	
     driver.findElement(By.cssSelector("#form__item__group")).sendKeys("edit_Ship Template group");
	
	String Edit_ShipTemplateGroup_name = driver.findElement(By.cssSelector("#form__item__group")).getText();
	...
	Reports.createNewReportTemplate_Reporttemplate_search(driver).sendKeys(Edit_ShipTemplate_name);
	
	Thread.sleep(6000);	
	
	String Edit_ShipTemplate_name_found = Reports.createNewReportTemplate_Reporttemplate_createdTemplate_namelink(driver).getText();
	
	Assert.assertEquals(Edit_ShipTemplate_name_found, Edit_ShipTemplate_name);
	
	String Edit_ShipTemplate_group_found = driver.findElement(By.xpath("/div/div[2]/div/div/div")).getText();
	
	Assert.assertEquals(Edit_ShipTemplate_group_found, Edit_ShipTemplateGroup_name);
	...
	Assert.assertEquals(Edit_ShipTemplate_name_found1, Edit_ShipTemplate_name);
	
	Assert.assertEquals(Edit_ShipTemplate_group_found1, Edit_ShipTemplateGroup_name);

	Log.info("Ship report template is updated successfully");	
